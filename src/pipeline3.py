    ...
    def run(self):
        ...
        if not self.skip_generation:
            if self.use_openai_in_generation:
                ...
                retrieved_contents = get_retrieved_outputs(f"truthfulqa_google_retrieval_top_30_post_post_trim_llama.out")
                assert len(self.data) == len(retrieved_contents)
                rag_inputs = generate_rag_find_inputs(self.data, retrieved_contents)
                model_outputs = run_llm(self.task+"-rag-google", rag_inputs, self.generation_model_name, max_tokens=128)
        else:
        ...
        facts_outputs = get_outputs(f"outputs/truthfulqa-rag-google-breakdown-para-llama3-gpt-3.5-turbo.out")
                ...
                for fact, verified_result in zip(facts, verified_results):
                    if correct_statement:
                        if "false" not in verified_result.lower():
                            ...
                            corrected_statement = run_openai(self.task + f"-statement_correct_{eid}", statement_correct_input, self.breakdown_model_name, max_tokens=128, save_to_file=False)#, loaded_model=llama_model, loaded_tokenizer=llama_tokenizer)
        ...
        if self.use_openai_in_correction:
            corrected_outputs = run_openai(self.task + "-rag-correct-statement-google-llama3-512-128", correction_inputs, self.correction_model_name, max_tokens=128)
        else:
            corrected_outputs = run_llm(self.task + "-rag-correct-statement-wq-google-512-128", correction_inputs, self.correction_model_name, max_tokens=128, loaded_model=llama_model, loaded_tokenizer=llama_tokenizer)
